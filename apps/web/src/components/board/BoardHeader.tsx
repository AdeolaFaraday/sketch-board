    ...
    <header className="flex items-center justify-between px-3 sm:px-5 py-2.5 sm:py-3 border-b border-white/[0.04] bg-slate-950/80 backdrop-blur-md shrink-0 relative z-20">
      <div className="flex items-center gap-2 sm:gap-4">
        ...
        <button
          ...
          className="lg:hidden p-2 rounded-xl text-slate-400 hover:bg-slate-800 hover:text-white transition-colors shrink-0"
          title="Members"
        >
      ...
      <div className="flex items-center gap-1.5 sm:gap-3">
        {/* Active board label – hidden on very small screens */}
        <div className="hidden md:flex items-center gap-2.5 bg-slate-800/60 border border-white/[0.06] rounded-xl px-3.5 py-1.5">
        ...
        <Button
          ...
          onClick={handleShare}
          className={`flex items-center gap-2 h-9 px-3 sm:px-4 text-[11px] font-bold uppercase tracking-wider transition-all duration-300 ${
            copied ? 'bg-emerald-600/20 text-emerald-400 border-emerald-500/30' : ''
          }`}
        >
          <ShareIcon className="w-3.5 h-3.5 shrink-0" />
          <span className="hidden xsm:inline">{copied ? 'Copied!' : 'Share'}</span>
        </Button>
